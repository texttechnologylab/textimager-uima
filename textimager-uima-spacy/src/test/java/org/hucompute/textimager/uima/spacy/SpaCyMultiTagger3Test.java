...
// English Tests =======================================================================================================
		...
		cas = JCasFactory.createText("This is an IPhone by Apple. And this is an iMac.", "en");

		//spacyMulti = createEngineDescription(SpaCyMultiTagger3.class,
		//		SpaCyMultiTagger3.PARAM_DOCKER_HOST_PORT, 8000
		//);
		SimplePipeline.runPipeline(cas, spacyMulti);
		...
		casTokens = (int[][]) JCasUtil.select(cas, Token.class).stream().map(s -> new int[] { s.getBegin(), s.getEnd() }).toArray(int[][]::new);
		...
		casSents = (int[][]) JCasUtil.select(cas, Sentence.class).stream().map(s -> new int[] { s.getBegin(), s.getEnd() }).toArray(int[][]::new);

		pos = new String[] {
				"DT", "VBZ", "DT", "NN", "IN", "NNP", ".", "CC", "DT", "VBZ", "DT", "NNP", "."
		};
		casPos = (String[]) JCasUtil.select(cas, POS.class).stream().map(p -> p.getPosValue()).toArray(String[]::new);

		deps = new String[] {
				"NSUBJ", "--", "DET", "ATTR", "PREP", "POBJ", "PUNCT", "CC", "NSUBJ", "--", "DET", "ATTR", "PUNCT"
		};
		casDeps = (String[]) JCasUtil.select(cas, Dependency.class).stream().map(p -> p.getDependencyType()).toArray(String[]::new);

		ents = new String[] { "ORG", "ORG" };
		casEnts = (String[]) JCasUtil.select(cas, NamedEntity.class).stream().map(p -> p.getValue()).toArray(String[]::new);
...
// French Tests =======================================================================================================
		...
		cas = JCasFactory.createText("Elle aime le chien. Marc a conduit la voiture.", "fr");
...
// Danish Tests =======================================================================================================
		...
		cas = JCasFactory.createText("Jeg så et hus. Han tog sin hat. Flyet er fløjet.", "da");
...
// Greek Tests =======================================================================================================
		...
		cas = JCasFactory.createText("πως σε λένε? Δεν καταλαβαίνω. Αγαπώ την Ελλάδα!", "el");
