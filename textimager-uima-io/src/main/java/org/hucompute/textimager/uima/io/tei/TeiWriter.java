...
extends JCasFileWriter_ImplBase
{
	...
	HashMap<Token, Integer>depIndex = new HashMap<>();
	HashMap<Token, Dependency>depIndexTokens = new HashMap<>();
			...
			while (iterator.isValid()) {
				...
				if (cur == null || nextAnnot.getBegin() < cur.getEnd()) {
					...
					if (cur == null || nextAnnot.getEnd() <= cur.getEnd()) {
						...
						if(cur.getTypeIndexID() == Sentence.type){
							List<Dependency> deps = JCasUtil.selectCovered(Dependency.class, cur);
							depIndex = new HashMap<>();
							depIndexTokens = new HashMap<>();
							for (int i = 0; i < deps.size(); i++) {
								depIndex.put(deps.get(i).getDependent(),i+1);
								depIndexTokens.put(deps.get(i).getDependent(),deps.get(i));
							}
						}
					}
					else {
	...
	public String getTei(JCas aJCas){
		cTextPattern = Pattern.compile("[,.:;()]|(``)|('')|(--)");
		String text = aJCas.getDocumentText();
...
//			}
		...
		//			Dependency c = (Dependency) aAnnotation;
		//			if ("ROOT".equals(c.getDependencyType())) {
		//				attributes.add(xmlef.createAttribute("depType", "--"));
		//			}
		//			if (c.getDependencyType() != null) {
		//				attributes.add(xmlef.createAttribute("depType", c.getDependencyType()));
		//			}
		//			attributes.add(xmlef.createAttribute("depIndex", Integer.toString(depIndex.get(c.getDependent()))));
		//			attributes.add(xmlef.createAttribute("parent", Integer.toString(depIndex.get(c.getGovernor()))));
		//
