...
import textimager.uima.io.abby.annotation.Block;
import textimager.uima.io.abby.annotation.Page;
import textimager.uima.io.abby.annotation.Paragraph;
import textimager.uima.io.abby.annotation.Token;
 ...
 */
public class DocumentReaderOneFileMultiPage extends JCasCollectionReader_ImplBase {
	/**
	...
	public void initialize(UimaContext context) throws ResourceInitializationException {
		...
		if(sourceLocation.endsWith("/")){
			throw new java.lang.RuntimeException("The file you specified looks like a directory.");
		}
		else {
	...
	public void process(JCas jCas, String inputFile, String documentId) throws IOException, ParserConfigurationException, SAXException {
...
//		}
		...
		FineReaderExportHandler fineReaderExportHandler = getExportHandler(saxParser, inputFile, pCharLeftMax, pBlockTopMin, lastTokenWasSpace);
...
//			strings.add(pages.get(pagePath).tokens.stream().map(Token::getTokenString).collect(Collectors.joining("")));
//		}
//		String text = textBuilder.toString();
		...
		if (documentId == null) {
			dmd.setDocumentId(inputFile.substring(inputFile.lastIndexOf("/")+1));
		}
		else {
			dmd.setDocumentId(documentId);
		}
//		dmd.setDocumentUri(inputFiles.get(0));
...
//		String pageInputPath = inputFiles.get(0);
//		FineReaderExportHandler fineReaderExportHandler = pages.get(pageInputPath);
		String pageId = Paths.get(inputFile).getFileName().toString();

		ArrayList<Page> pageArrayList = fineReaderExportHandler.pages;
		System.out.println(pageArrayList.size());
		System.out.println("---------------------------------------");
		for (int j = 0; j < pageArrayList.size(); j++) {
			Page page = pageArrayList.get(j);
			System.out.println(page.start + " " + page.end);
			page.pageId = pageId + "_" + j;
			page.pageNumber = runningPageNumer++;
			OCRPage ocrPage = page.wrap(jCas, lastOffset);
			ocrPage.setBegin(page.start);
			jCas.addFsToIndexes(ocrPage);
			System.out.println("Prossing page from " + ocrPage.getBegin() + " to " + ocrPage.getEnd() + " offset " + lastOffset);
//				lastOffset = ocrPage.getEnd();
//				System.out.println(lastOffset + " " + ocrPage.getEnd());
//				System.out.println();
//			}
...
//				String description = String.format("letterRatio:%03f, charactersPerToken:%03f, alnumRatio:%03f, weirdNumberTable:%b", letterRatio, charactersPerToken, alnumRatio, !(numberTable && letterRatio >= pMinLineLetterRatio * 2));
	...
	public void getNext(JCas jCAS) throws IOException, CollectionException {
		try {
			...
			jCAS.setDocumentLanguage(language);
			process(jCAS, inputFile, documentId);
			stillHasNext = false;
