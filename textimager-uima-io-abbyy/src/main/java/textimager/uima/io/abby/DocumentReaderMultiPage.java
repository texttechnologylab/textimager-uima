 ...
 */
public class DocumentReaderMultiPage extends JCasCollectionReader_ImplBase {
	/**
	...
	public void initialize(UimaContext context) throws ResourceInitializationException {
		...
		if(sourceLocation.endsWith("/")){
			try {
				Files.list(Paths.get(sourceLocation)).forEachOrdered(f->inputFiles.add(f.toString()));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		else {
			inputFiles.add(sourceLocation);
		}
...
//		}
		...
		if (documentId == null) {
			dmd.setDocumentId(inputFiles.get(0).substring(inputFiles.get(0).lastIndexOf("/")+1));
		}
		else {
...
//		System.out.println("---------------------------------------");
		for (int j = 0; j < pageArrayList.size(); j++) {
			...
			String zdtn = ZonedDateTime.now().format(DateTimeFormatter.RFC_1123_DATE_TIME).toString();
			System.out.println(zdtn + " " + page.start + " " + page.end);
			page.pageId = pageId + "_" + j;
			page.pageNumber = runningPageNumer++;
			OCRPage ocrPage = page.wrap(jCas, lastOffset);
//			System.out.println("Prossing page from " + ocrPage.getBegin() + " to " + ocrPage.getEnd());
//			System.out.println("Prossing page from " + page.start + " to " + page.end);
//			System.out.println("");
//			ocrPage.setBegin(page.start);
//			ocrPage.setEnd(page.end);
